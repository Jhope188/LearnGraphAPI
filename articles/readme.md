...
| Articles Hub | `conditionalaccess.tech/articles/` | `articles/index.html` |
...
## 📚 Articles Hub — `articles/index.html`

Tabbed article index with a **Latest Article** banner at the top. Three series tabs:
...
- **Feature grid** — every feature row with issue number, date, badge type, and article link
...
Autodiscovery `<link>` tags are present in both `index.html` and `articles/index.html`. An RSS icon in the articles nav links directly to the feed.
...
```
articles/
  index.html                  ← Articles hub (tabbed, latest banner)
  readme.md                   ← This file
...
| File | What changes |
|---|---|
| `articles/index.html` | Latest-banner updated, tab count +1, coming-soon card → live |
| `index.html` | Featured article resource-card title, description, URL |
| `feed.xml` | New `<item>` prepended, `lastBuildDate` updated |
| `entra-news.html` | New feature row added, Features + Issues counts incremented |
| `index.html` | Entra.News badge count updated |
