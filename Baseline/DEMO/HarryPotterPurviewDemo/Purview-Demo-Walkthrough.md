...
|---|-------------|---------------|--------|
| 1 | **Sensitivity Labels prerequisites enabled** | Run `Enable-SensitivityLabelsPrerequisites.ps1` — enables MIP labels for Groups, SharePoint, and syncs to Entra ID | ☐ |
| 2 | **SharePoint sensitivity labels enabled** | Run `Enable-SharePointSensitivityLabels.ps1` — sets `EnableAIPIntegration = $true` | ☐ |
| 3 | **24-hour propagation complete** | Both scripts require up to 24 hrs to fully propagate | ☐ |
| 4 | **Copilot for M365 licences assigned** | Assign licences to Dolores Umbridge + at least one Gryffindor user | ☐ |
| 5 | **Harry Potter users exist** | 21 member users created via `Create-HarryPotterUsers.ps1` | ☐ |
| 6 | **SharePoint sites exist** | 20+ sites from `Create-HarryPotterDemoEnvironment.ps1` + Order of the Phoenix from `Create-HarryPotterSharePointDemo.ps1` | ☐ |
| 7 | **Gryffindor dynamic group populated** | 6 members (see below) | ☐ |
