...
- **Viewport meta must disable pinch-zoom on every article using this pattern:** `<meta name="viewport" content="width=device-width, initial-scale=1.0, maximum-scale=1.0, user-scalable=no">`. Without this, pinch-zooming on a real device causes `position: fixed` elements (the topbar and the sticky mobile nav) to detach from the visual viewport and misalign or disappear — the page "loses its layout" the moment a user zooms out, even though nothing is visually wrong at 1:1 zoom. This is a real device-only bug that never reproduces in desktop dev tools resizing, only in an actual pinch-zoom gesture (or a tester/emulator that supports it).
